(ns cateringops.operation
  ...
  (CateringAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the CateringGovernor (:govern)
  ...
  human operator/catering coordinator. The approver resumes with
...
(defn build
  ...
  (-> (g/state-graph
      ...
      ;; CateringAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; CateringGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
